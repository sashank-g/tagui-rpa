...
techo('https://www.google.com/' + ' - ' + this.getTitle() + '\n');});

casper.then(function() { // start of JS code
as = 'AA'
}); // end of JS code

// to use variable we need to use template literal syntax
casper.then(function() {{techo('type q as '+as+' [enter]');
casper.waitFor(function check() {return check_tx('q');},
function then() {this.sendKeys(tx('q'),''+as+' ',{keepFocus: true});
this.sendKeys(tx('q'),casper.page.event.key.Enter,{keepFocus: true});
this.sendKeys(tx('q'),'',{keepFocus: true});},
function timeout() {this.echo('ERROR - cannot find q').exit();});}});

casper.then(function() {this.echo('+as+');});
...
casper.then(function() {{techo('snap page to results.png');
this.capture('E:/PS/Tag/results.png');}});
